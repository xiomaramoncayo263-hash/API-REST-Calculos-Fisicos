...
function EnergiaCinetica() {
    ...
    const calcularEnergiaCinetica = async () => {
        ...
        fetch("http://localhost:3000/fisica/energiaCinetica", {
            method: "POST",
            ...
            <div className="bg-white/95 w-full max-w-md rounded-2xl border-2 border-gray-400 shadow-2xl p-8">
                ...
                <div className="space-y-4">
                    ...
                    <div>    
                        ...
                        <input 
                            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" type="number" placeholder="Ingrese el valor de la velocidad" onChange={(e) => SetVelocidad(Number(e.target.value))}
                        />
                    </div>

                    <button 
                        onClick={calcularEnergiaCinetica} className="w-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition duration-200">
                        Calcular
                    </button>
